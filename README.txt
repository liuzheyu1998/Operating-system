Student name: Zheyu Liu
ID: 260784914

OS used: mimi.cs.mcgill.ca

Special instructions:
1. As professor specified in Piazza, the quit command will exit script only, not the shell,  when we run a script with quit command. So for exec command, when one file has a quit command, then the rest of this file will not be executed, but other files will still be executed.
